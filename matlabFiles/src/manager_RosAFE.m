...
%             end
        ...
        function deleteProcessor( mObj, procName )
            ...
            %            
            remove = mObj.RosAFE.removeProcessor( procName );
        ...
        function processChunk( mObj )
            ...
            for ii = 1:length(mObj.Processors.input)
                sigName = mObj.Processors.input{ii}.name;
                inputSig = output.result.signals.input( ii );

                if ( inputSig{1}.framesOnPort > 0 )
                    mObj.dObj.(sigName){1}.appendChunk( cell2mat(inputSig{1}.left.data)' );
                    mObj.dObj.(sigName){2}.appendChunk( cell2mat(inputSig{1}.right.data)' );
                end
            end
            
            % Pre Processors  
            for ii = 1:length(mObj.Processors.preProc)
                sigName = mObj.Processors.preProc{ii}.name;
            ...
            for ii = 1:length(mObj.Processors.gammatone)
                sigName = mObj.Processors.gammatone{ii}.name;
                inputSig = output.result.signals.gammatone( ii );

                if ( inputSig{1}.framesOnPort > 0 )

                	[chunkLeft chunkRight] = adaptTFS( inputSig{1}.framesOnPort, inputSig{1}.numberOfChannels, inputSig{1}.left, inputSig{1}.right, 1 );
                    mObj.dObj.(sigName){1}.appendChunk( chunkLeft );
                    mObj.dObj.(sigName){2}.appendChunk( chunkRight );
                end       
            end
            % IHC
            for ii = 1:length(mObj.Processors.ihc)
                sigName = mObj.Processors.ihc{ii}.name;
            ...
            for ii = 1:length(mObj.Processors.ild)
                sigName = mObj.Processors.ild{ii}.name;
                inputSig = output.result.signals.ild( ii );

                if ( inputSig{1}.framesOnPort > 0 )

                	chunkLeft = adaptTFS( inputSig{1}.framesOnPort, inputSig{1}.numberOfChannels, inputSig{1}.left, inputSig{1}.right, 0 );
                    mObj.dObj.(sigName){1}.appendChunk( chunkLeft );
                end       
            end             
        end    
        ...
        function addSingleProcessor( mObj, procType, procName, dependencies, par )
            ...
            switch ( procType )
                ...
                case ('preProc')
                    ...
                    sig_l = TimeDomainSignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, ...
                            name, 'Pre processor', 'left');
                    sig_r = TimeDomainSignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, ...
                            name, 'Pre processor', 'right');
                ...
                case ('gammatoneProc')
                    ...
                    sig_l = TimeFrequencySignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, name, 'Gammatone Proc', cell2mat(mObj.Processors.gammatone{this_jj}.fb_cfHz), 'left');
                    sig_r = TimeFrequencySignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, name, 'Gammatone Proc', cell2mat(mObj.Processors.gammatone{this_jj}.fb_cfHz), 'right');
                ...
                case ('ihcProc')
                    ...
                    sig_l = TimeFrequencySignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, name, 'IHC Proc', cell2mat(mObj.Processors.gammatone{this_jj}.fb_cfHz), 'left');
                    sig_r = TimeFrequencySignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, name, 'IHC Proc', cell2mat(mObj.Processors.gammatone{this_jj}.fb_cfHz), 'right');
                ...
                case ('ildProc')
                    ...
                    sig_mono = TimeFrequencySignal.construct(mObj.dObj.sampleRate, mObj.dObj.bufferSize_s_matlab, name, 'ILD Proc', cell2mat(mObj.Processors.gammatone{this_jj}.fb_cfHz), 'mono');
